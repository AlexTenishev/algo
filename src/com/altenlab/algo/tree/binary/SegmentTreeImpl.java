 ...
 * Memory usage:  O(n)
 *
 * @author Ricardo Pacheco, modified by A.T.
 */
//FIXME: This is very-very rough draft
public class SegmentTreeImpl {
    ...
    static class Node {
        ...
        public String toString() {
            return "From: "+from+", To: "+to+", size: "+size()+", accumulator: "+accumulator +", extremmum: "+extremmum+", pendingVal: "+(pendingVal != null ? pendingVal : "NULL");
        }
    }
     ...
     */
    public SegmentTreeImpl(final int[] array) {
    ...
    private void build(int v, int from, int size) {
        ...
        if (size == 1) {
            heap[v].accumulator = array[from] != 0 ? 1 : 0;
            heap[v].extremmum = array[from];
        } else {
            //Build childs
            build(2 * v, from, size / 2);
            build(2 * v + 1, from + size / 2, size - size / 2);

            heap[v].accumulator = heap[2 * v].accumulator * heap[2 * v + 1].accumulator;
            //min = min of the children
            heap[v].extremmum = Math.max(heap[2 * v].extremmum, heap[2 * v + 1].extremmum);
        }
    }
    ...
    private void update(int v, int from, int to, int value) {
        ...
        if (intersects(from, to, n.from, n.to)) {
            ...
            n.accumulator = heap[2 * v].accumulator * heap[2 * v + 1].accumulator;
            n.extremmum = Math.max(heap[2 * v].extremmum, heap[2 * v + 1].extremmum);
        }
    }
    ...
    private void change(Node n, int value) {
        n.pendingVal = value;
        n.accumulator = n.size() * value; //FIXME?
        n.extremmum = value;
        array[n.from] = value;
